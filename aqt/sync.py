...
from aqt.qt import QDialog, QDialogButtonBox, QGridLayout, QLabel, QLineEdit, \
    QObject, QThread, QVBoxLayout, SIGNAL
import aqt
...
Please visit AnkiWeb, upgrade your deck, then try again."""))
    ...
    def _rewriteError(self, err):
        if "Errno 61" in err:
            return _("""Couldn't connect to AnkiWeb. Please check your \
network connection and try again.""")
        elif "timed out" in err:
            return _(""" The connection to AnkiWeb timed out. Please check \
your network connection and try again.""")
        elif "500" in err:
            return _("""AnkiWeb encountered an error. Please try again in a \
few minutes, and if the problem persists, please file a bug report.""")
        elif "501" in err:
            return _("Please upgrade to the latest version of Anki.")
        # 502 is technically due to the server restarting, but we reuse the
        # error message
        elif "502" in err or "503" in err or "504" in err:
            return _(""" AnkiWeb is too busy at the moment. Please try again \
in a few minutes.""")
        elif "409" in err:
            return _(
...
enter your details below.""") %
    ...
    def _confirmFullSync(self):
        diag = askUserDialog(
            _("""Your decks here and on AnkiWeb differ in such a way that \
they can't be merged together, so it's necessary to overwrite the \
decks on one side with the decks from the other.
